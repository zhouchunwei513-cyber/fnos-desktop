...
1. **"渲染进程路由跳转加载应用"已按需求落实（v2.4.2）**：`open-fpk-app` found:true 时渲染进程
   在主窗口内 `location.assign(url)` 同窗打开应用页面（程序内启动，≡主页点击应用图标），
   不再由主进程新建独立应用窗口；应用页主导航完成（did-navigate）后显示主窗口防主页闪现。
   应用页面运行在主窗口 preload 环境（非 subapp-preload），回主页走应用内导航或托盘【显示主界面】。
...
7. **主窗口显示策略（v2.4.1/v2.4.2，用户反馈覆盖 v2.4.0 恒 hide）**：点击启动飞牛主程（无应用
   参数）显示主窗口（主页，含 10s 黑屏兜底）；桌面快捷方式（`--launch-app`/`--app`/`--open-app`）
   在主窗口内同窗打开应用（程序内启动，主窗口内容即应用、不显示主页）；开机自启（Run 键命令带
   `--autostart` 标记、不带应用 ID）隐藏主窗口只驻留托盘（需求 2.2 不变）。
