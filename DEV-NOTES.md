...
- **v2.4.13**（2026-09-23，zip 配置修复）：v2.4.12 Build 步秒挂真凶=package.json 顶层 `"zip"` 配置键在 electron-builder 26.x schema 非法（run 164 实录）；改为 zip artifactName 内联 target 条目、构建命令配置驱动（CLI 不传 --win），本地验证双产物后发布。发布链取证与 zip 双轨策略同 v2.4.12。
- **v2.4.14**（2026-09-23，r15 应用窗口类型探测+快捷方式启动分支（硬规则））：窗口类型识别（需求四-1 元信息优先：fntb /api/client/apps + NAS /app/list 双通道、字段多候选兼容 ui_config 嵌套；元信息缺失走网页实际加载行为探测兜底：setWindowOpenHandler 观测回填）→ windowType 特征标记持久缓存（apps-manifest.json：windowType/windowTypeSrc/windowTypeAt，refreshFpkApps 登录后 debounce 自动探测）→ 快捷方式启动分支（__openAppByWindowType：内嵌窗型=壳 iframe 宿主窗 embed-shell.html 内嵌渲染到桌面，硬规则绝不剥离 fnOS 框架转独立跳出窗、禁止顶层直接访问原始应用 URL；非内嵌窗型=直接弹出独立窗口）。createAppWindow 加载收口 __appLoadTarget（embedStyle→loadFile 壳页）；内嵌窗标题栏移除 ↗ 跳出按钮（五钮→四钮）；__notifyOpenFpkApp payload 增 windowType；无标记走 __fnosLaunchByClick 行为兜底自动适配；零新增 IPC。并入 v2.4.13 zip 配置修复（顶层 zip 非法键删除、artifactName 内联、构建命令配置驱动）。
- **v2.4.15**（2026-09-23，r15b 快捷方式启动单窗口体验）：快捷方式打开应用后飞牛主页面隐藏到托盘（hideMainToBackground，默认 tray/设置可选 minimize），桌面只呈现应用窗——内嵌=壳 iframe 窗前置（show+focus）、跳出=独立窗；修复 r15 主页面与应用窗两窗口并存问题。
- **v2.4.16**（2026-09-24，r15c 真机日志修复）：windowType 缓存键归一化（trim.docker↔docker 孤儿条目）+探测缓存真实 URL（embed 壳窗加载真实应用地址，appview 禁入壳防递归）；快捷方式启动藏主页面全覆盖（无标记兜底也 hideMainToBackground）；cache.get 诊断埋点。
- **v2.5.3**（2026-09-24，r19 实测修复 4 问题）：①黑屏根修=渲染进程崩溃（日志实锤全部应用窗 create 后 10-300ms `render-gone crashed exitCode 0x80000003`，对照组无 FPK 图标窗正常加载——GIF/WebP/损坏 FPK 图标进 nativeImage→win.setIcon 触发崩溃）：新增 __safeIconBuf 签名校验（PNG/JPEG/ICO/BMP 白名单），fpk-early 窗口图标链/favicon 链/pngToIco 入口三处拦截，非白名单跳过留默认图标；②从主页面打开应用主页面不藏托盘（__openAppByWindowType 增 opts.keepMain：setWindowOpenHandler 链+appview 内嵌转出链不再 hide；快捷方式 launch.http/second-instance/冷启动链保持藏托盘单窗体验不变）；③白纸图标根修=FPK 主链 pngToIco 非 PNG 输入校验（fndesk GIF 374KB→坏 ICO 实锤，r18 只修了回退链漏了 FPK 主链）+非 PNG 落 exe 兜底+ico 文件名带内容 hash 头 6 位绕开 Windows 图标缓存+pngToIco 空输出防护；④秒开：崩溃风暴引发 crash-storm-delay 2500ms 延迟随①消除；launch 防抖命中改为聚焦已有同应用窗（不再无响应），窗不存在放行重开不误杀。
- **v2.5.2**（2026-09-24，r18 实测修复 5 问题 + 新需求）：①入口分流修正（解析/清单 URL 优先直开、appview?anchor 仅兜底——修"应用不存在或未安装"：v2.5.1 无条件构造 appview 覆盖 Fndesk=/app/fndesk、FNTB=host:18080、OpenList=独立服务、Lucky=/app/Lucky/ 等真实 URL；trim.* 系统应用继续走清单 appview 容器 PreAuth 不变）；②appview 容器页 chrome 清理（右上假窗口按钮=双标题栏、左侧 dock 黑块=显示模式问题、左下头像浮钮）+ 唯一 iframe 撑满修布局塌缩型黑屏 + 状态埋点；③快捷方式动态图标兜底（GIF/WebP 转换失败不再塞无效 IconLocation=白纸图标根因，全链失败兜底 exe 自带图标）；④新需求已实现并增强：点主程序图标/快捷方式（托盘或任务栏运行中）→ second-instance → __restoreMainHome 强制回桌面刷新（r17 M7），本版锁定态也前置显示锁定页。
- **v2.5.1**（2026-09-24，r17 实测修复 7 问题）：①统一 appview 容器入口（跳出窗首选 `appview?anchor=appId`，服务端 PreAuth 完整——直开真实 URL 丢登录态跳登录页/黑屏/找不到地址的根修；无 appId 才真实 URL 直开）；②双窗消除（launch.http per-appId 3s 防抖 + 同应用窗复用聚焦 + notify windowType 必非空杜绝 preload 模拟点击 fallback 双开）；③主窗恢复统一回桌面刷新（托盘点击/双击/菜单/双击主程序 __restoreMainHome——hide 后渲染冻结黑屏/桌面图标不渲染的根修）；④应用窗加载失败耗尽原生弹窗（可重试/关闭）；⑤昼夜主题跟随主窗（nativeTheme.themeSource 同步）；⑥preload window.open 透传 fnos-anchor。⚠️注意：r17 日志实测为 v2.4.19（embed-shell 埋点实锤），7 问题中双窗叠加/PreAuth 丢失/主窗黑屏/主题不同步在 v2.5.0 亦存在，均在本版修复。
- **v2.5.0**（2026-09-24，r16 方案定案：统一"从主程序跳出的窗口"）：用户放弃内嵌单窗方案——不论内嵌/跳出窗型一律独立跳出窗，全程单窗口（主程序藏托盘、禁止双窗）；内嵌窗监测自动转跳出（preload iframe 监测→window.open→跳出窗+移除内嵌容器；新增 appview 路由型监测→真实 URL 跳出窗+静默关闭主窗内嵌视图）；window.open 跳出型行为不变；壳 iframe（embed-shell）停用；快捷方式直开统一跳出（真实 URL 兜底链保留）。
- **v2.4.19**（2026-09-24，r15f 黑屏修复+直开 URL 兜底）：①输错服务器地址黑屏——did-fail-load 重试耗尽原生弹窗报错+自动回连接页重新输入（旧逻辑静默黑屏卡死）；连接页加载两级失败改原生弹窗+3s 自动重试；②首击 2 窗根修——preset 种子带实测真实 URL（origin 相对路径）+元信息通道缓存带 url+__openAppByWindowType fntb 清单 URL 兜底链，embed 直开不再回落模拟点击；③manifest 列表刷新合并保留 windowType（换服务器/扫描重建缓存不再丢失）；④切换服务器立即重探测窗口类型（preset 重种）。
- **v2.4.18**（2026-09-24，r15e 元信息直读定案）：FPK ui/config 入口配置直读窗口类型（entry.type: iframe|url，官方打包规范）+ 8 样本实例实测分类固化为首击种子 + app-center 静态服务 ui/config 多路径通道（带登录 cookie）；fntb v2.18.8 起 /api/client/apps 透传 type。修复 v2.4.17 真机 meta:0→首击全 miss→回落模拟点击（窗口中间态）的根因：已知应用首击即命中直开（内嵌=壳 iframe 单窗前置/跳出=独立窗），主页面藏托盘，不再模拟点击。
- **v2.4.17**（2026-09-24，r15d 收口定案）：窗口类型分支+藏主页面收口到快捷方式真实入口 launch.http handler（vbs→HTTP /launch 直发 notify 的链路此前完全绕过 r15 分支/hide，真机日志实证）；内嵌=壳 iframe 壳窗、跳出=独立应用窗、无标记回落模拟点击兜底+观测回填；任何快捷方式启动主页面藏托盘/最小化。
