...
```bash
pnpm install
# 本地构建需先准备 mpv：把 mpv.exe（及 d3dcompiler_43.dll）放到项目根目录 mpv/
#   可从 https://sourceforge.net/projects/mpv-player-windows/files/64bit/ 下载解压
pnpm start      # 开发运行
pnpm dist       # 打包便携版 -> dist/FNOS-<version>-portable.exe
```

> mpv.exe 约 120MB，超过 GitHub 单文件 100MB 限制，故不入库；GitHub Actions 构建时自动从本仓库 `mpv-runtime` Release 下载官方 mpv（GPL）。

**自动构建发布**：push `v*` 标签（如 `v1.27.1`）即触发 Actions：装依赖 → 下载 mpv 运行时 → electron-builder 打包 → 发布到该 tag 的 Release 附件。也可在 Actions 页手动 Run workflow。
