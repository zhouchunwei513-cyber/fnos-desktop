...
// ==================== v2.6.0 r26（问题1/3/4）：无感假窗控清除 + 白底统一 + 早触发 ==========
// r25 误杀修正版聚组识别（KEEP 排除自有 DOM）+ 三处升级：
// ① 已处理元素 data-fnos-killed26 跳过（不再重复显示/隐藏→消除"杀的过程可见"）；
// ② 夜间模式下白底（rgb 全通道>230）统一改 #12141a（混搭白屏字段看不见的直接根因）；
// ③ 触发提前到 did-dom-ready + 100ms（配合 preload document_start 即时扫描实现首帧前无感）。
const __FNOS_FRAME_SCRIPT26 = [
  '(function(){try{',
  'var D=__DARK24__;var out={h:0,bg:0,sub:(window.top!==window),href:String(location.href||"").slice(0,80)};',
  'var KEEP="[data-fnos-tb],[id^=fnos-],[class*=fnos-]";',
  'try{var r=document.documentElement;',
  'r.setAttribute("data-fnos-theme",D?"dark":"light");r.setAttribute("data-theme",D?"dark":"light");',
  'r.style.colorScheme=D?"dark":"light";',
  ...
  'try{if(document.body&&document.body.classList){document.body.classList.remove(D?"light":"dark");document.body.classList.add(D?"dark":"light");}}catch(_){}',
  '}catch(_){}',
  'try{if(D){var bg="";try{bg=(document.body&&getComputedStyle(document.body).backgroundColor)||"";}catch(_){}',
  'var m=/rgba?\\((\\d+)\\s*,\\s*(\\d+)\\s*,\\s*(\\d+)/.exec(bg);',
  ...
  'ez.setAttribute("data-fnos-killed26","1");ez.style.setProperty("display","none","important");out.h++;}catch(_){}}}',
  '}catch(_){}}',
  '}catch(_){}',
  'return out;',
  '}catch(e){return{err:String(e).slice(0,80)}}})()'
].join('');
function __frameSweep26(win, tag) {
  try {
    if (!win || win.isDestroyed() || !win.webContents || win.webContents.isDestroyed()) return;
    const mf = win.webContents.mainFrame;
    if (!mf) return;
    ...
    for (const f of frames) {
      try {
        ...
        f.executeJavaScript(__script).then((r) => {
          try {
            __done++;
            if (r && typeof r === 'object') { __hidden += (r.h | 0); if (r.sub) __sub++; __bg += (r.bg | 0); }
            if (__done >= frames.length) {
              fnosLog('info', 'frame-inject26', { winId: win.id, tag: String(tag || ''), frames: frames.length, hidden: __hidden, sub: __sub, bgFixed: __bg, theme: __FNOS_APP_DARK ? 'dark' : 'light', ms: Date.now() - __t0 });
            }
          } catch (_) {}
        }).catch(() => {
          try { __done++; if (__done >= frames.length) fnosLog('warn', 'frame-inject26', { winId: win.id, tag: String(tag || ''), frames: frames.length, hidden: __hidden, partial: true }); } catch (_) {}
        });
      } catch (_) {}
    }
    if (!frames.length) { try { fnosLog('info', 'frame-inject26', { winId: win.id, tag: String(tag || ''), frames: 0 }); } catch (_) {} }
  } catch (e) { try { fnosLog('warn', 'frame-inject26', { winId: win && win.id, err: String(e && e.message || e).slice(0, 120) }); } catch (_) {} }
}
function __armFrameInject26(win, tag) {
  try {
    if (!win || win.isDestroyed() || win.__fnFrameInj26) return;
    win.__fnFrameInj26 = true;
    const __sweep = () => { try { __frameSweep26(win, tag); } catch (_) {} };
    try { win.webContents.on('did-dom-ready', __sweep); } catch (_) {}
    try { win.webContents.on('did-finish-load', __sweep); } catch (_) {}
    try { win.webContents.on('did-frame-finish-load', __sweep); } catch (_) {}
    try { win.webContents.on('did-frame-navigate', __sweep); } catch (_) {}
    try { [100, 1200, 3200, 8000, 15000, 25000].forEach((ms) => setTimeout(__sweep, ms)); } catch (_) {}
  } catch (_) {}
}
// v2.6.0 r26（问题2）：manifest 归一查询——URL 候选把 trim.docs 污染成 trim-docs、
// 桌面图标按 alt=显示名匹配，点击兜底必须用 manifest 原始 appId+显示名双键查找。
function __fnosManifestLookup26(q) {
  try {
    const qq = String(q || '').toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]/g, '');
    if (!qq) return null;
    let mf = null;
    try { mf = readManifest(); } catch (_) {}
    const apps = (mf && mf.apps) || [];
    for (const a of apps) {
      if (!a) continue;
      for (const f of ['appId', 'appName', 'name', 'url']) {
        const v = String(a[f] || '').toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]/g, '');
        if (v && v === qq) return { appId: String(a.appId || a.appName || q), label: String(a.appName || a.name || '') };
      }
    }
  } catch (_) {}
  return null;
  ...
  win.webContents.on('render-process-gone', (_e, details) => {
    try {
      ...
      dlog && dlog('error', 'appwin.render-gone.register', {
        ...
        // v2.5.6 r22：崩溃现场采集（问题8 黑屏风暴根因下轮定位——fpk-skip 未 setIcon 也崩=
        // 非 setIcon 真凶；second-instance 后 5-8s 新窗风暴强相关）
        procCount: (() => { try { return require('electron').app.getAppMetrics().length; } catch (_) { return -1; } })(),
        memMB: (() => { try { return Math.round(process.getSystemMemoryInfo().total / 1024); } catch (_) { return -1; } })(),
        winCount: (() => { try { return BrowserWindow.getAllWindows().filter((w) => !w.isDestroyed()).length; } catch (_) { return -1; } })(),
      });
    } catch (_) {}
    if (entry.isHome) {
  ...
  try {
    if (!win.__fnLoadStall25) {
      ...
      try {
        ...
        // v2.6.0 r26（问题4）：页面加载完成≠应用就绪——LUN/文本编辑器/预览实测 did-finish-load 后
        // 内容区 loading 转圈不止（应用内 API 挂起）。12s 后仍见可见 spinner → 埋点+单次 reload 自愈。
        win.webContents.on('did-finish-load', () => {
          setTimeout(() => {
            try {
              if (win.isDestroyed() || win.webContents.isDestroyed()) return;
              win.webContents.executeJavaScript('(function(){try{var s=document.querySelectorAll(".semi-spin,.ant-spin,[class*=\"loading\" i],[class*=\"spinner\" i],[class*=\"skeleton\" i]");for(var i=0;i<s.length;i++){var r=s[i].getBoundingClientRect();if(r.width>24&&r.height>24&&r.top<window.innerHeight){return 1}}return 0}catch(_){return 0}})()').then((st) => {
                try {
                  if (!st) return;
                  if (!win.__fnInnerStall26) {
                    win.__fnInnerStall26 = true;
                    require('./logger.js').log('warn', 'appwin', 'appwin.inner-stall', { params: { winId: win.id, url: String(win.webContents.getURL() || '').slice(0, 120), action: 'reload-once' } }, __RUN_MODE);
                    try { win.webContents.reload(); } catch (_) {}
                  } else {
                    require('./logger.js').log('warn', 'appwin', 'appwin.inner-stall2', { params: { winId: win.id, url: String(win.webContents.getURL() || '').slice(0, 120) } }, __RUN_MODE);
                  }
                } catch (_) {}
              }).catch(() => {});
            } catch (_) {}
...
function createAppWindowInner(url, opts = {}, __cw_t0 = Date.now()) {
  ...
  try {
    ...
    if (__origR21 && /^https?:/i.test(__origR21)) {
      // v2.7.0 r27（问题②④）：NAS 真入口注册表候选一律置顶——isDisplay=false 文件关联应用
      // （trim.docs/text-editor/preview）桌面无图标，一切猜测候选均不可靠；真入口=origin+path 直拼。
      try {
        const __nmT27 = __mAnchor21 ? decodeURIComponent(__mAnchor21[1]) : (__mRoute21 ? decodeURIComponent(__mRoute21[1]) : '');
        const __trT27 = __entryTruth27(__nmT27) || __entryTruth27(String((opts && (opts.name || opts.appId)) || ''));
        if (__trT27) {
          const __uT27 = __trT27.port
            ? (String(__trT27.protocol || 'http:') + '//' + (new URL(__origR21).hostname) + ':' + __trT27.port + __trT27.path)
            : (String(__origR21).replace(/\/+$/, '') + __trT27.path);
          __urlCandidates.push(__uT27);
          try { dlog && dlog('info', 'appwin.entry-truth27', { name: __trT27.name, url: String(__uT27).slice(0, 120) }); } catch (_) {}
        }
      } catch (_) {}
      // v2.5.6 r22：候选扩展——FPK 图标管理器真入口 url 字段+app-center live URL 优先
      // （trim.iscsi→/iscsi/、fndesk→/app/fndesk、FNTB→:18080 各 app 真入口形态完全不同，
      // appview?anchor 与 /app/{kebab} 都是猜的=trim.docs"跳转了也不起作用"实锤），再按
      // appview?anchor-kebab↔/app/{kebab}↔/app/{原名} 全排列逐一试。
      const __pushFpkUrl22 = (nmX) => {
        try {
          const fe = __fpkLookupSync(nmX) || __fpkLookupSync(String(nmX || '').replace(/\./g, '-'));
          if (fe && fe.url && /^https?:/i.test(String(fe.url))) __urlCandidates.push(String(fe.url));
        } catch (_) {}
      };
      if (__mAnchor21) {
        ...
        try {
          ...
          if (fe23 && fe23.url && String(fe23.url).charAt(0) === '/') {
            if (fe23.port) {
              ...
              // v2.7.0 r27：port 空（native/CGI/主站路由型）真入口=origin+path 主站同源直拼
              // （trim-cli 实测 text-editor CGI/trim.docs /docs/preview 全 200/403=服务在监听，
              // ERR_CONNECTION_REFUSED 根因=旧条件 fe23.port 把这些应用排挤进猜测候选）
              __urlCandidates.push(String(__origR21).replace(/\/+$/, '') + String(fe23.url));
            }
          }
        } catch (_) {}
        try {
        ...
        try {
          ...
          if (fe23g && fe23g.url && String(fe23g.url).charAt(0) === '/') {
            if (fe23g.port) {
              ...
              // v2.7.0 r27：route 分支同——port 空真入口=origin+path 主站同源直拼
              __urlCandidates.push(String(__origR21).replace(/\/+$/, '') + String(fe23g.url));
            }
          }
        } catch (_) {}
        try {
  ...
  try {
    ...
    win.webContents.on('did-finish-load', () => {
      ...
      try { __armFrameInject26(win, 'app:' + String(__appLabel || '')); } catch (_) {}
      // v2.6.0 r26（问题5 日志）：导航失败全程埋点（-102 连接拒绝等根因直击加载挂起）
      try {
        win.webContents.on('did-fail-load', (_e, code, desc, url, isMainFrame) => {
          try { require('./logger.js').log('warn', 'appwin', 'appwin.nav-error', { params: { winId: win.id, code: code, desc: String(desc || '').slice(0, 80), url: String(url || '').slice(0, 120), main: !!isMainFrame } }, __RUN_MODE); } catch (_) {}
        });
      } catch (_) {}
                  ...
                  try {
                    ...
                    const __tryNext23 = async () => {
                      ...
                      if (!win.__fnClickFallback25) {
                        ...
                        // v2.6.0 r26（问题2）：appId 归一回 manifest 原始 ID + 显示名——URL 候选把
                        // trim.docs 污染成 trim-docs、桌面图标按 alt=显示名匹配，双偏差导致 icon-not-found
                        // 后又开 appview 错误页（死循环）。归一后走原始 ID + 显示名双键点击。
                        let __clickMeta26 = null;
                        try { __clickMeta26 = __fnosManifestLookup26(__clickApp25) || __fnosManifestLookup26(__appLabel); } catch (_) {}
                        if (__clickMeta26 && __clickMeta26.appId) { try { __clickApp25 = __clickMeta26.appId; } catch (_) {} }
                        ...
                        try { __notifyOpenFpkApp(__clickApp25, true, String(win.webContents.getURL() || ''), '', (__clickMeta26 && __clickMeta26.label) ? [__clickMeta26.label] : []); } catch (_) {}
                        setTimeout(() => {
                          try {
                            if (win.isDestroyed()) return;
                  ...
                  (async () => {
                    try {
                      ...
                      if (__fpkName) {
                        ...
                        // v2.5.6 r22：FPK 图标全格式转干净 PNG（canvas 万能转换+磁盘缓存）后进 nativeImage
                        let __fpkSafe = null;
                        try { __fpkSafe = fpkBuf ? (await __decodeToPngAny(fpkBuf) || null) : null; } catch (_) {}
...
function __entryErrorPage23(name) {
  // v2.7.0 r27：显示名映射（trim.docs→Office文档、fn-ups-manager→UPS 电源管理——旧文案
  // 错报"应用 trim docs 不存在"实锤）+ UPS 专属引导（真入口在系统设置面板，非桌面应用）
  const __trE27 = __entryTruth27(name);
  const nm = ((__trE27 && __trE27.name) || String(name || '应用')).replace(/[<>&"']/g, '');
  const __hint27 = (/ups/i.test(String(name || '')) || (__trE27 && __trE27.hint === 'settings'))
    ? '<p style="opacity:.85;margin:8px 0 0">提示：UPS 电源管理的入口在【系统设置 &rarr; UPS】，请从系统设置打开。</p>' : '';
  return '<!DOCTYPE html><html><head><meta charset="utf-8"><title>应用 ' + nm + ' 不可用</title></head>'
    ...
    + '<p style="opacity:.85;margin:0 0 6px">所有启动入口均返回&ldquo;不存在或未安装&rdquo;。</p>' + __hint27
    ...
    for (const a of list) {
      ...
      try {
        ...
        if ((pp === '' || pp === '/') && !pu.search && !pu.hash) {
          try { fnosLog('info', 'shortcut.url', 'app-center live root-url kept', { name: nm, url: String(u).slice(0, 100) }); } catch (_) {}
...
function __safeDecodeImageToPng(buf) {
  // v2.5.6 r22：全格式统一转干净 PNG（问题5 根修：jpg/webp 直接落 LNK=白纸图标实锤）。
  // PNG：nativeImage 验证+重编码（坏 PNG 拒绝）；jpg/ico/bmp：nativeImage 解码 toPNG
  // （官方 nativeImage 支持 PNG/JPEG/ICO）；webp/gif/svg：磁盘缓存→WIC 兜底（canvas
  // 异步万能转换见 __decodeToPngAny）。r22 实锤 WIC 全挂（PresentationCore 对 WebP 无
  // 解码器+大图 15s 超时，12 次 status:1）→换 nativeImage/canvas 通道。
  try {
    if (!buf || buf.length < 20) return null;
    const kind = __safeIconBuf(buf);
    if (kind === 'png') {
      try {
        const img = nativeImage.createFromBuffer(Buffer.from(buf));
        if (!img.isEmpty()) {
          const p = img.toPNG();
          if (p && p.length > 100) return Buffer.from(p);
        }
      } catch (_) {}
      try { fnosLog('warn', 'icon.decode', 'png verify failed', { size: buf.length }); } catch (_) {}
      return null;
    }
    if (kind === 'jpg' || kind === 'ico' || kind === 'bmp') {
      try {
        const img = nativeImage.createFromBuffer(Buffer.from(buf));
        if (!img.isEmpty()) {
          const p = img.toPNG();
          if (p && p.length > 100) {
            try { fnosLog('info', 'icon.decode', 'native decode to png', { from: kind, size: buf.length, pngSize: p.length }); } catch (_) {}
            return Buffer.from(p);
          }
        }
      } catch (_) {}
    }
    // v2.5.6 r22：磁盘转换缓存优先（同 icon 只转一次，WIC 不再每次开窗重跑卡 7 秒=不秒开真凶）
    try {
      const __cp22 = __iconConvCachePath(buf);
      if (__cp22 && fs.existsSync(__cp22)) {
        const __cb22 = fs.readFileSync(__cp22);
        if (__cb22.length > 100 && __safeIconBuf(__cb22) === 'png') {
          try { fnosLog('info', 'icon.decode', 'conv cache hit', { kind: kind || 'unknown' }); } catch (_) {}
          return __cb22;
        }
      }
    } catch (_) {}
    ...
    if (png && __safeIconBuf(png) === 'png') {
      ...
      try { fnosLog('info', 'icon.decode', 'wic converted to png', { from: kind || 'unknown', size: buf.length, pngSize: png.length }); } catch (_) {}
      return png;
    }
    try { fnosLog('warn', 'icon.decode', 'decode failed', { size: buf.length, kind: kind || 'unknown', magic: Buffer.from(buf.slice(0, 8)).toString('hex') }); } catch (_) {}
    return null;
  } catch (_) { return null; }
}
// v2.5.6 r22：canvas 万能转 PNG（Chromium 解码器支持 WebP/JPEG/GIF/BMP）+磁盘缓存。
// r22 日志实锤：WIC（PowerShell PresentationCore）对 WebP 无解码器、550KB jpg 15s 超时
// 全挂（wic run failed status:1 x12）→keep source file→假 .png 进 LNK=白纸；且每次开窗
// 重跑 WIC 卡 7 秒=不秒开真凶。同 icon 只转一次（ASSETS_DIR/.iconconv/{md5}.png）。
function __iconConvCachePath(buf) {
  try {
    const crypto = require('crypto');
    const h = crypto.createHash('md5').update(Buffer.from(buf)).digest('hex');
    return path.join(ASSETS_DIR, '.iconconv', h + '.png');
  } catch (_) { return ''; }
}
...
async function __decodeToPngAny(buf) {
  // v2.5.6 r22：全格式异步统一转换入口——同步层(png验证/jpg/ico/bmp)→磁盘缓存→canvas→WIC
  try {
    if (!buf || buf.length < 20) return null;
    const kind = __safeIconBuf(buf);
    if (kind === 'png' || kind === 'jpg' || kind === 'ico' || kind === 'bmp') {
      const sync = __safeDecodeImageToPng(buf);
      if (sync && __safeIconBuf(sync) === 'png') return sync;
    }
    try {
      const cp = __iconConvCachePath(buf);
      if (cp && fs.existsSync(cp)) {
        const b = fs.readFileSync(cp);
        if (b.length > 100 && __safeIconBuf(b) === 'png') {
          try { fnosLog('info', 'icon.decode', 'conv cache hit', { kind: kind || 'unknown' }); } catch (_) {}
          return b;
        }
      }
    } catch (_) {}
    let png = await __canvasDecodeToPng(buf);
    if (png && __safeIconBuf(png) === 'png') {
      try {
        const cp = __iconConvCachePath(buf);
        if (cp) { fs.mkdirSync(path.dirname(cp), { recursive: true }); fs.writeFileSync(cp, png); }
      } catch (_) {}
      try { fnosLog('info', 'icon.decode', 'canvas converted to png', { kind: kind || 'unknown', size: buf.length, pngSize: png.length }); } catch (_) {}
      return png;
    }
    png = __wicToPng(buf);
    if (png && __safeIconBuf(png) === 'png') return png;
    try { fnosLog('warn', 'icon.decode', 'all decode failed', { kind: kind || 'unknown', size: buf.length }); } catch (_) {}
    return null;
  } catch (_) { return null; }
}
...
ipcMain.handle('create-desktop-shortcut', async (_e, payload) => {
  try {
    ...
    if (fpkAppName) {
      ...
      // v2.5.6 r22：全格式转干净 PNG（canvas 万能转换，jpg/webp 直接落 LNK=白纸实锤）；
      // 转换后仍非 PNG 才走"保留原图"分支。ico 文件名带内容 hash 头 6 位，绕开 Windows 图标缓存。
      let __fpkConv22 = null;
      try { __fpkConv22 = fpkBuf && fpkBuf.length > 0 ? await __decodeToPngAny(fpkBuf) : null; } catch (_) {}
      if (fpkBuf && fpkBuf.length > 0 && !(__fpkConv22 && __safeIconBuf(__fpkConv22) === 'png')) {
      ...
      if (resolvedIconPath && fs.existsSync(resolvedIconPath)) {
        try {
          ...
          if (ext === '.ico') {
            ...
            // v2.5.6 r22：canvas 万能转换归一 PNG（WIC 全挂+WebP 无解码器实锤）；真损坏才保留原图
            try { const __nC = await __decodeToPngAny(imgBuf); if (__nC) imgBuf = __nC; } catch (_) {}
...
function __notifyOpenFpkApp(appId, found, url, windowType, labels) {
  try {
    const payload = {
      ...
      labels: Array.isArray(labels) ? labels.slice(0, 4) : [],
      ts: Date.now(),
    };
    const __send = () => {
      try {
...
function __wtKeyNorm(s) {
  // 键归一化：trim.docker ↔ docker ↔ Trim.Docker（大小写/分隔符/trim. 前缀），修复观测写入键
  // （fpkAppNameFromUrl 的 URL 反查名如 "docker"）与快捷方式查询键（appId "trim.docker"）
  // 不一致导致缓存永远命不中的问题（真机日志 v2.4.15 实证：cache.set 后 Get 仍为空）。
  return String(s || '').toLowerCase().replace(/^trim[._-]/, '').replace(/[._\-]/g, '');
}
function __windowTypeCache(appName, type, src, realUrl) {
  try {
    if (!appName || (type !== 'embed' && type !== 'popout')) return;
    const mf = readManifest();
    if (!mf || !Array.isArray(mf.apps)) return;
    const nk = __wtKeyNorm(appName);
    const hits = mf.apps.filter((a) => a && ((a.appId && __wtKeyNorm(a.appId) === nk) || (a.name && __wtKeyNorm(a.name) === nk)));
    if (!hits.length) {
      const e = { appId: String(appName), name: String(appName) };
      mf.apps.push(e);
      hits.push(e);
    }
    let changedAny = false;
    for (const e of hits) {
      if (e.windowType !== type) { e.windowType = type; changedAny = true; }
      if (e.windowTypeSrc !== src) { e.windowTypeSrc = src; changedAny = true; }
      const ru = String(realUrl || '').slice(0, 400);
      if (ru && e.windowTypeUrl !== ru) { e.windowTypeUrl = ru; changedAny = true; }
    }
    if (changedAny) {
      for (const e of hits) { e.windowTypeAt = Date.now(); }
      writeManifest(mf);
      try { require('./logger.js').log('info', 'wintype', 'cache.set', { params: { app: String(appName).slice(0, 120), type, src, url: String(realUrl || '').slice(0, 140), matched: hits.length } }, __RUN_MODE); } catch (_) {}
    }
  } catch (_) {}
}
function __windowTypeGetEntry(appName) {
  try {
    if (!appName) return null;
    const mf = readManifest();
    const nk = __wtKeyNorm(appName);
    const e = mf && Array.isArray(mf.apps) ? mf.apps.find((a) => a && a.windowType && ((a.appId && __wtKeyNorm(a.appId) === nk) || (a.name && __wtKeyNorm(a.name) === nk))) : null;
    const r = e ? { type: String(e.windowType), url: String(e.windowTypeUrl || '') } : null;
    try { require('./logger.js').log('info', 'wintype', 'cache.get', { params: { q: String(appName).slice(0, 120), hit: !!r, type: r ? r.type : '' } }, __RUN_MODE); } catch (_) {}
    return r;
  } catch (_) { return null; }
}
function __windowTypeGet(appName) {
  const r = __windowTypeGetEntry(appName);
  return r ? r.type : '';
}
function __windowTypeObserve(url, type) {
  try {
    const name = fpkAppNameFromUrl(String(url || '')) || '';
    if (name && (type === 'embed' || type === 'popout')) __windowTypeCache(name, type, 'behavior', String(url || ''));
  } catch (_) {}
}
...
function __openAppByWindowType(appId, appName, appUrl, windowType, realUrl, opts) {
  ...
  try {
    ...
    // v2.7.0 r27：清单/解析全灭 → NAS 真入口注册表兜底（origin+path 直拼；isDisplay=false
    // 文件关联应用与 fn-ups-manager 类无 entry 应用的唯一活路，替代 appview?anchor 猜词）
    if (!cand) {
      try {
        const __trR27 = __entryTruth27(nm) || __entryTruth27(String(appId || ''));
        if (__trR27 && /^https?:/i.test(String(origin || ''))) cand = String(origin).replace(/\/+$/, '') + __trR27.path;
        if (cand) { try { dlog && dlog('info', 'resolve.truth27', { name: __trR27.name, url: String(cand).slice(0, 120) }); } catch (_) {} }
      } catch (_) {}
    }
    if (cand) target = cand;
...
// v2.7.0 r27（问题②④根治数据）：NAS 真入口注册表——trim-cli `app entry list` 实测 2026-09-26。
// trim.docs 真入口 entryName=trim.docs.preview（type=url，/docs/preview，isDisplay=false 65 种
// fileTypes 文件关联，桌面无图标）；fn-ups-manager 无 entry 无 service 仅 CGI；text-editor=CGI path。
// URL 铁则：port 非空 {proto}://{host}:{port}{path}；port 空={origin}{path} 主站同源直拼（实测全 200）。
const __FNOS_ENTRY_TRUTH27 = [
  { keys: ['trim.docs', 'trim-docs', 'trim.docs.preview', 'trim-docs-preview', 'office文档'], name: 'Office文档', path: '/docs/preview', port: '', protocol: '', displayable: false },
  { keys: ['fn-ups-manager', 'fnupsmanager', 'ups电源管理'], name: 'UPS 电源管理', path: '/cgi/ThirdParty/fn-ups-manager/index.cgi/', port: '', protocol: '', displayable: false, hint: 'settings' },
  { keys: ['trim.text-editor', 'trim-text-editor', 'trim.texteditor', '文本编辑器'], name: '文本编辑器', path: '/cgi/ThirdParty/trim.text-editor/index.cgi/', port: '', protocol: '', displayable: false },
  { keys: ['trim.preview', 'trim-preview'], name: '预览', path: '/app/trim-preview/', port: '', protocol: '', displayable: false },
  { keys: ['trim.iscsi', 'trim-iscsi'], name: 'iSCSI', path: '/iscsi/', port: '', protocol: '', displayable: true },
];
function __entryNorm27(s) {
  try { return String(s || '').toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]/g, ''); } catch (_) { return ''; }
}
function __entryTruth27(q) {
  try {
    const nq = __entryNorm27(q);
    if (!nq) return null;
    for (const t of __FNOS_ENTRY_TRUTH27) {
      for (const k of t.keys) {
        const nk = __entryNorm27(k);
        if (!nk) continue;
        if (nq === nk || (nq.length >= 3 && (nq.indexOf(nk) >= 0 || nk.indexOf(nq) >= 0))) return t;
      }
    }
    return null;
  } catch (_) { return null; }
