...
--[[
  ============================================================================
  fnOS 中文 OSC 定制层（基于 mpv v0.41 官方 osc.lua，仅追加/汉化，不改布局逻辑）
  - 界面汉化：缓存时长、音轨/字幕/倍速/画中画等中文标签
  - 功能增补：倍速菜单、音轨选择菜单、字幕设置菜单（含在线字幕搜索）、画中画开关
  - 菜单统一复用 mpv 内置 @context_menu.lua（menu-data + script-message open），
    全鼠标可操作；本脚本不引入任何第三方依赖。
  ============================================================================
]]
...
function FN.zh(style_str)
    -- style_str 形如 "...\\fs20}"；在结尾 } 前插入字体覆盖
    if not style_str then return "\\fs18\\fn" .. FN.cjk_font .. "}" end
...
function FN.lang_of(t)
    local lang = (t.lang and t.lang ~= "") and t.lang or "未知"
    local title = (t.title and t.title ~= "") and (" " .. t.title) or ""
    return lang .. title
end
...
function FN.menu_speed()
    ...
    local function is_c(v) return math.abs(cur - v) < 0.001 end
    local items = {
        { title = "播放倍速（当前 " .. string.format("%.2f", cur) .. "x）",
          state = { "disabled" }, cmd = "osd-msg show-text 播放倍速" },
        { type = "separator" },
    }
    local speeds = { 0.5, 0.75, 1.0, 1.25, 1.5, 2.0 }
    for _, s in ipairs(speeds) do
        local label
        if math.abs(s - 1.0) < 0.001 then label = "1.0x（正常）" else label = string.format("%.2gx", s) end
        local states = {}
        if is_c(s) then states = { "checked" } end
        items[#items + 1] = {
            title = label,
...
function FN.build_sub_items(tracks)
    ...
    items[#items + 1] = {
        ...
        cmd = "set sid no; osd-msg show-text 关闭字幕"
    }
    for _, t in ipairs(tracks) do
        local sel = (t.id == cur and sub_visible)
        items[#items + 1] = {
            title = "字幕轨 " .. t.id .. " · " .. FN.lang_of(t) .. (sel and "  [当前]" or ""),
            state = sel and { "checked" } or nil,
            cmd = "set sid " .. t.id .. "; set sub-visibility yes; osd-msg show-text 字幕轨 " .. t.id
        }
    end
    items[#items + 1] = { type = "separator" }
    ...
    items[#items + 1] = { title = "字幕放大", cmd = "add sub-scale 0.1; osd-msg show-text 字幕放大" }
    items[#items + 1] = { title = "字幕缩小", cmd = "add sub-scale -0.1; osd-msg show-text 字幕缩小" }
    items[#items + 1] = { title = "字幕大小重置", cmd = "set sub-scale 1; osd-msg show-text 字幕大小已重置" }
    local pos = mp.get_property_number("sub-margin-y", 25) or 25
    items[#items + 1] = { title = "字幕位置（当前底距 " .. tostring(math.floor(pos)) .. "）", state = { "disabled" },
        cmd = "osd-msg show-text 字幕位置" }
...
local function bar_layout(direction, slim)
    ...
    local tsW = 110
    local minW = (buttonW + padX)*5 + (tcW + padX)*4 + (tsW + padX)*2 + 66*2
...
local function osc_init()
    ...
    --audio_track（fnOS：左键弹中文音轨菜单，右键循环切换）
    ...
    --sub_track（fnOS：左键弹中文字幕菜单，右键循环切换）
    ...
    --fnOS 新增：倍速按钮（纯中文文字按钮）
    ne = new_element("fnos_speed", "button")
    ne.enabled = true
    ne.content = function ()
    ...
    ne.content = function ()
        ...
        -- fnOS 汉化：缓存时长
        if min > 0 then
            return "缓存: " .. string.format("%d 分 %02d 秒", min, sec)
        else
            return "缓存: " .. string.format("%d 秒", sec)
        end
    end
